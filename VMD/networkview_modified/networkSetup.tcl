...
proc loadFirstTrajectoryFrameByPDB {dcdFile pdbFile} {
    mol load pdb $pdbFile
    mol addfile $dcdFile last 0
...
proc loadTrajectoryFilesBYPDB {dcdFile pdbFile} {
...
proc prepTrajectoryFilesByPDB {dcdFileNames selString dcdFileName psfFileName pdbFileName indexFileName stride} {
    set sel [atomselect top $selString]
    $sel writepsf $psfFileName
    cleanPsf $psfFileName
    $sel writepdb $pdbFileName
...
proc networkSetup {paramFileName} {
    ...
    set carmaPdbFileName "carma.pdb"
    ...
    while {![eof $paramFile]} {
       if {[regexp {>Psf} $line]} {
           ...
           set skip [string trim [getParameterLine $paramFile]]
    ...
    if { ![string equal $pdbFileName ""] &&
       ...
       return
               ...
               ![string equal $dcdFileNames ""] &&
                [string equal $psfFileName ""]} {
      loadFirstTrajectoryFrameByPDB $firstDcdFileName $pdbFileName
      prepTrajectoryFilesByPDB $dcdFileNames $nodeSelString $carmaDcdFileName $carmaPsfFileName $carmaPdbFileName $carmaIndexFileName 1
      runCarma $carmaDcdFileName $carmaPsfFileName
      prepTrajectoryFilesByPDB $dcdFileNames $systemSelString $adjmatDcdFileName $adjmatPsfFileName $adjmatPdbFileName $adjmatIndexFileName $skip
      loadTrajectoryFilesBYPDB $adjmatDcdFileName $adjmatPdbFileName
      getAdjacencyMatrix top $paramFileName "contact" "carma.fitted.dcd.varcov.dat"
      return
      }
